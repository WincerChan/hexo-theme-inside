  ...
  const site = this.config,
    ...
    config = Object.assign(
      ...
      pick(theme, ['profile', 'menu', 'footer', 'comments', 'post']),
      {
        ...
        routes: ["life", "about", "about-the-question", "friends"],
        count: {
          posts: locals.posts.filter(post => !post.hide).length,
          categories: locals.categories.length,
          // filter unpublished posts
          tags: locals.tags.filter(tag => tag.posts.filter(post => !post.hide).length).length
        },
