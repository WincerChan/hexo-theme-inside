...
module.exports = function ({ theme, locals, helpers }) {
  const tags = locals.tags.filter(tag => tag.posts.filter(post => !post.hide).length).sort('name');
  ...
  return flattenDeep([
    helpers.generateJson({
      path: 'tags',
      data: tags.map(tag => ({ name: tag.name, count: tag.posts.filter(p => !p.hide).length }))
    }),
    ...
    tags.map(tag => [
      helpers.pagination.apply(
        tag.posts.sort('-date').filter(post => !post.hide).map(pick(tagPostsProps)),
